...
int	get_tex(t_main *main, t_img *img, int i, double wall_h)
{
	...
	main->inv_tile = 1.0 / TILE;
	main->img_width_inv_tile = img->width * main->inv_tile;
	if (wall_h > HEIGHT)
		i += (int)((wall_h - HEIGHT) * 0.5);
	if (main->ray->flag == 1)
		x = (int)(main->ray->p_x * main->img_width_inv_tile) % img->width;
	else
		x = (int)(main->ray->p_y * main->img_width_inv_tile) % img->width;
	y = (int)(i * (img->height / wall_h));
