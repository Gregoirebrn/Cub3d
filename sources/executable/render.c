...
/*   Updated: 2024/09/30 17:07:23 by beroy            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_wall(t_main *main, int ray, double wall_h)
{
	int	color;
	int	i;
	int	b_pix;
...
void	render_ray(t_main *main, int ray)
{
	...
	wall_h = (TILE / main->ray->distance)
		* ((WIDTH / 2) / tan(main->plyr->fov_rd / 2));
	b_pix = (HEIGHT / 2) + (wall_h / 2);
	t_pix = (HEIGHT / 2) - (wall_h / 2);
	if (b_pix > HEIGHT)
		b_pix = HEIGHT;
